...
def safe_get_value_or_number(data, key, default="No Information"):
    """Handle cases where value might be an object with 'value' key or just a number"""
    item = data.get(key, default)
    if item == default:
        return default
    
    # If it's a dict, try to get 'value' key
    if isinstance(item, dict):
        return item.get("value", default)
    
    # Otherwise return the item itself (it's probably a number)
    return item
...
def convert(llm_output_dict):
    row = {
        ...
        "fee": number_conversion(safe_get_value_or_number(llm_output_dict, "brokerageFee", 0)),
